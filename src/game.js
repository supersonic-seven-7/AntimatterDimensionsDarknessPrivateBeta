...
export function breakInfinity() {
  if (!Autobuyer.bigCrunch.hasMaxedInterval || (!UltimateChallenge(1).isCompleted && PlayerProgress.eternityUnlocked())) return;
...
export function gainedEternityPoints() {
  let ep = DC.D5.pow(player.records.thisEternity.maxIP.plus(
    gainedInfinityPoints()).log10() / (EternityChallenge(5).reward.effectOrDefault(3083) - PelleRifts.recursion.effectValue.toNumber()) - 0.7).times(totalEPMult());
...
export function gameLoop(passDiff, options = {}) {
  ...
  if (!Achievement(188).isUnlocked && !player.anticheat) {
